...
DECLARE_DELEGATE(FTimePointEventSignature);
DECLARE_DELEGATE_OneParam(FTimeLoopEventSignature, float);
...
public:
	...
	FTimeLoopEventSignature LoopEvent;
...
protected:
	...
	TMap<FName, FTimePointEvent> Events;
...
public:	
	...
	*/
	UFUNCTION(BlueprintPure, Category = "NaPack|Component")
	void AddTimePointEvent(FName Name, meta = (DefaultToSelf), float TimeSecondsFromNow, const FTimePointEventSignature & Event, bool bForceAdd = true);

	/* Add loop event using time seconds from now as the first execution time.
	* @Param ForceAdd If true, when the name is existing, the new event will override the old one. Or new event will not be added if the name is existing.
	*/
	UFUNCTION(BlueprintPure, meta = (DefaultToSelf), Category = "NaPack|Component")
	void AddLoopEvent(FName Name, float TimeSecondsFromNow, const FTimeLoopEventSignature & Event, bool bForceAdd = true);
